...
class Grid:
    ...
    def _create_line_between_cells(self, selected_cells):
        if len(selected_cells) != 2:
            print("Error: This algorithm requires exactly two cells to draw a line.")
            return
        
        cell1, cell2 = selected_cells[0], selected_cells[1]
        
        # Calculate the line coordinates using Bresenham's line algorithm
        x1, y1 = cell1
        x2, y2 = cell2
        dx = abs(x2 - x1)
        dy = abs(y2 - y1)
        ...
        # Render the line on the grid by rendering each cell along the line
    ...
    def _redraw(self):
        ...
        for i in range(dimension):
            for j in range(dimension):
                ...
                elif self.raster.filled_cells[x][y]:
                    color = '#00ffff'
                    text_color = ''
                    text = ''
